...
if ( ! function_exists( 'school_hub_get_default_theme_options' ) ) :
	 ...
	 */
	function school_hub_get_default_theme_options() {
		...
		$defaults['ticker_title']          = esc_html__( 'News:', 'school-hub' );
		...
		// Footer.
		$defaults['copyright_text']        = esc_html__( 'Copyright &copy; All rights reserved.', 'school-hub' );
		$defaults['show_social_in_footer'] = false;

		// Blog.
		$defaults['excerpt_length']     = 40;
		$defaults['read_more_text']     = esc_html__( 'Read more', 'school-hub' );
		...
		$defaults['news_and_events_ntitle']    = esc_html__( 'News', 'school-hub' );
		$defaults['news_and_events_nnumber']   = 2;
		$defaults['news_and_events_ncategory'] = 0;
		$defaults['news_and_events_etitle']    = esc_html__( 'Events', 'school-hub' );
		$defaults['news_and_events_enumber']   = 4;
		$defaults['news_and_events_ecategory'] = 0;

		// Homepage Call To Action.
		$defaults['cta_title']               = esc_html__( 'Join Our school', 'school-hub' );
		$defaults['cta_description']         = esc_html__( 'Every undergraduate student is eligible to receive a fellowship of up to $10,000 for a summer internship or faculty-mentored research project. Find your opportunity, make your case for how it fits your academic plans, and we will help fund it.', 'school-hub' );
		$defaults['cta_primary_button_text'] = esc_html__( 'Learn More', 'school-hub' );
		$defaults['cta_primary_button_url']  = '#';
		$defaults['cta_secondary_button_text'] = esc_html__( 'Online Tour', 'school-hub' );
		$defaults['cta_secondary_button_url']  = '#';

		// Homepage Latest News.
		$defaults['latest_news_title']          = esc_html__( 'Latest News', 'school-hub' );
		$defaults['latest_news_layout']         = 1;
		$defaults['latest_news_category']       = 0;
		$defaults['latest_news_number']         = 4;
		$defaults['latest_news_column']         = 4;
		$defaults['latest_news_featured_image'] = 'school-hub-thumb';
		...
		$defaults['featured_slider_read_more_text']      = esc_html__( 'Read More', 'school-hub' );

		// Pass through filter.
		$defaults = apply_filters( 'school_hub_filter_default_theme_options', $defaults );
		return $defaults;
	}
